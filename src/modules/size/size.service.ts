...
export class SizeService {
  ...
  async findOne(id: string): Promise<Size> {
    const res: Size = await this.repository.findOne(id);
  ...
  async create(size: Partial<Size>): Promise<Size> {
    ...
    if (!res)
      throw new InternalServerErrorException('No se pudo crear el talle');
    return res;
  }

  async update(id: string, size: Partial<Size>): Promise<UpdateResult> {
    const res = await this.repository.update(id, size);
    if (res.affected === 0)
      throw new NotFoundException('No se encontro el talle a actualizar');
    return res;
  }
  ...
  async remove(id: string): Promise<DeleteResult> {
    try {
      ...
      if (res.affected === 0)
        throw new NotFoundException('No se encontro el talle');
      return res;
    } catch (e) {
      throw new ConflictException(
        `No se puede eliminar el talle porque esta relacionado con otra informacion y dejaria inconsistencias. Pruebe modificandolo. `,
      );
    }
  }
}
