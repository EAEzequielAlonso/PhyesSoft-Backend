...
export class SaleProductsController {
  ...
  @Get("sale/:saleId")
  async getSaleProductsBySale (@Param("saleId", ParseUUIDPipe) saleId:string): Promise<SaleProducts[]> {
      return await this.saleProductsService.getSaleProductsBySale(saleId)
  }

  @Post()
  async createSaleProducts (@Body() saleProducts:CreateSaleProductDto): Promise<SaleProducts> {
