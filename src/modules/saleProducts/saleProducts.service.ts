...
export class SaleProductsService {
  constructor(
    private readonly saleProductsRepository: SaleProductsRepository,
  ) {}
  ...
  async createSaleProducts(
    saleProducts: Partial<SaleProducts>,
  ): Promise<SaleProducts> {
    const saleProductCreate: SaleProducts =
      await this.saleProductsRepository.createSaleProducts(saleProducts);
    if (!saleProductCreate)
      throw new InternalServerErrorException(
        'No se pudo agregar el producto a la venta',
      );
