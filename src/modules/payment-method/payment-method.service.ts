...
export class PaymentMethodService {
  ...
  async findCemmerce(commerceId: string): Promise<PaymentMethod[]> {
    try {
      ...
      if (!res)
        throw new NotFoundException('No se encontraron los metodos de pago');
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
  ...
  async update(id: string, method: Partial<PaymentMethod>) {
    try {
      ...
      if (res.affected === 0)
        throw new NotFoundException('No se pudo encontrar el metodo de pago');
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
  ...
  async remove(id: string) {
    try {
      ...
      throw new ConflictException(
        'Actualmente el metodo de pago esta siendo usado. No se puede eliminar ya que dejaría información incompleta. Puede probar modificando el metodo de pago si lo cree necesario. Error: ',
        error,
      );
    }
  }
