...
export class SizeTypeService {
  ...
  async findCemmerce(commerceId:string): Promise<SizeType[]> {
    try {
      ...
      if (!res) throw new NotFoundException("No se encontraron los grupos de Talles")
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error)
    }
  ...
  async findOne(id: string): Promise<SizeType> {
    try {
      ...
      if (!res) throw new NotFoundException("No se encontro el grupo de Talles")
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error)
    }
  ...
  async create(sizeType: Partial<SizeType>): Promise<SizeType> {
    try {
      ...
      if (!res) throw new InternalServerErrorException("No se pudo crear el grupo de Talles")
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error)
    }
  ...
  async update(id: string, sizeType: Partial<SizeType>) {
    try {
      ...
      if (res.affected === 0) throw new NotFoundException("No se pudo encontrar el grupo de Talles")
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error)
    }
  ...
  async remove(id: string) {
    try {
      ...
      throw new ConflictException(
        'Actualmente el grupo de Talles esta siendo usado. No se puede eliminar ya que dejaría información incompleta. Puede probar modificando el grupo de Talles si lo cree necesario. Error: ',error);
    }
  }
}
