...
export class CashMovementService {
  ...
  async findAll(dailyCashId: string, pageNumber:number,
    ...
    try {  
      const response = await this.repository.findAll(dailyCashId, pageNumber,
        limitNumber,
        search);
      return response 
    } catch (error) {
  ...
  async create(body: Partial<CashMovement>): Promise<CashMovement> {
    try {
      ...
      if (!dailyCash) throw new InternalServerErrorException(`No se encontró la caja diaria`);
  ...
  async update(id: string, body: Partial<CashMovement>) {
    try {
      ...
      if (!cashMov) throw new InternalServerErrorException(`No se pudo encontrar ${this.completeMessage}`);
      ...
      if (+cashMov.amount != +body.amount) {
        // 1. Leer el dailyCash actual
        const dailyCash = await this.dailyCashrepository.findOne(body.dailyCashId);
        if (!dailyCash) throw new InternalServerErrorException(`No se encontró la caja diaria`);

        // 2. Actualizar sumando los valores nuevos
        await this.dailyCashrepository.update(body.dailyCashId, {
          movements: (+dailyCash.movements || 0) + ((+body.amount-(+cashMov.amount) ) || 0),
        });
      }
  ...
  async remove(id: string) {
    try {
      ...
      // 1. Leer el dailyCash actual
      const dailyCash = await this.dailyCashrepository.findOne(cashMov.dailyCashId);
      if (!dailyCash) throw new InternalServerErrorException(`No se encontró la caja diaria`);
