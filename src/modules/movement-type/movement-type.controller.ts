import { Controller, Get, Post, Body, Param, Delete, UseGuards, Req, Query, ParseUUIDPipe, Put } from '@nestjs/common';
   ...
   constructor(private readonly service: MovementTypeService) {}
    ...
    @Get()
      ...
      async findAll(
          ...
          @Query('search') search = ''): Promise<[MovementType[], number]> {
      ...
      @Post()
      async create(@Body() method: CreateMovementTypeDto, @Req() req:Request): Promise<MovementType> {
      ...
      @Put(':id')
      update(@Param('id', ParseUUIDPipe) id: string, @Body() method: UpdateMovementTypeDto) {
