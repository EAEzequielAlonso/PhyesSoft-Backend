...
export class MovementTypeService {
    ...
    try {
      ...
      if (!res)
        throw new NotFoundException(
          'No se encontraron los tipos de movimientos',
        );
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
  ...
  async findOne(id: string): Promise<MovementType> {
    try {
      ...
      if (!res)
        throw new NotFoundException('No se encontro el tipo de movimiento');
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
    ...
    try {
      ...
      if (!res)
        throw new InternalServerErrorException(
          'No se pudo crear el tipo de movimiento',
        );
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
  ...
  async update(id: string, mov: Partial<MovementType>) {
    try {
      ...
      if (res.affected === 0)
        throw new NotFoundException(
          'No se pudo encontrar el tipo de moviemiento',
        );
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
  ...
  async remove(id: string) {
    try {
      ...
      throw new ConflictException(
        'Actualmente el tipo de movimiento esta siendo usado. No se puede eliminar ya que dejaría información incompleta. Puede probar modificando el tipo de movimiento si lo cree necesario. Error: ',
        error,
      );
    }
  }
