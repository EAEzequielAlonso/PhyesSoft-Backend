...
export class SaleRepository {
    ...
    async getSales (startDate?: Date, endDate?: Date): Promise<Sale[]> {
        if (startDate) {
            if (endDate) {
                return await this.saleRepository.find({ where: {date: Between (startDate, endDate) }});
            } else {
                return await this.saleRepository.find({ where: {date: Equal (startDate) }});
            }
        }
    ...
    async getSalesByClient (clientId:string, startDate?: Date, endDate?: Date): Promise<Sale[]> {
        if (startDate) {
            if (endDate) {
                return await this.saleRepository.find({ where: {clientId, date: Between (startDate, endDate) }});
            } else {
                return await this.saleRepository.find({ where: {clientId, date: Equal (startDate) }});
            }
        }
    ...
    async getSalesByBranch (branchId:string, startDate?: Date, endDate?: Date): Promise<Sale[]> {
        if (startDate) {
            if (endDate) {
                return await this.saleRepository.find({ where: {branchId, date: Between (startDate, endDate) }});
            } else {
                return await this.saleRepository.find({ where: {branchId, date: Equal (startDate) }});
            }
        }
