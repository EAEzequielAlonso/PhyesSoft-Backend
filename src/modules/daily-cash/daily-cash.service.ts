...
export class DailyCashService {
  ...
  async findOpen(commerceId: string): Promise<DailyCash[]> {
    try {
      ...
      if (!res)
        throw new NotFoundException('No se encontraron cajas diarias abiertas');
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
  ...
  async update(id: string, body: Partial<DailyCash>) {
    try {
      ...
      if (res.affected === 0)
        throw new NotFoundException('No se pudo encontrar la caja diaria');
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
  ...
  async remove(id: string) {
    try {
      ...
      throw new ConflictException(
        'Actualmente la caja diaria esta siendo usado. No se puede eliminar ya que dejaría información incompleta. Puede probar modificando la caja diaria si lo cree necesario. Error: ',
        error,
      );
    }
  }
