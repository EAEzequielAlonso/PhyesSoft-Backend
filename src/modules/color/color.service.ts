...
export class ColorService {
  ...
  async updateColor(id: string, color: Partial<Color>): Promise<Color> {
    ...
    if (colorUpdate.affected === 0)
      throw new NotFoundException('No se encontro el color a actualizar');
  ...
  async deleteColor(id: string): Promise<Color> {
    ...
    if (!color)
      throw new NotFoundException('No se encontro el color a eliminar');
