...
export class ColorService {
  ...
  async create(color: Partial<Color>): Promise<Color> {
    try {
      ...
      if (!colorRes) throw new InternalServerErrorException("No se pudo crear el Color")
      return colorRes;
    } catch (error) {
      throw new InternalServerErrorException(error)
    }
  ...
  async update(id: string, color: Partial<Color>) {
    try {
      ...
      if (colorRes.affected === 0) throw new NotFoundException("No se pudo encontrar el Color")
      return colorRes;
    } catch (error) {
      throw new InternalServerErrorException(error)
    }
  ...
  async remove(id: string) {
    try {
      ...
      throw new ConflictException(
        'Actualmente la categoria esta siendo usada. No se puede eliminar ya que dejaría información incompleta. Puede probar modificando la categoria si lo cree necesario. Error: ',error);
    }
  }
}
