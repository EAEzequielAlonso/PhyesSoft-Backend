...
export class ValueVariantService {
  ...
  async findByProductVariant(Id: string): Promise<ValueVariant[]> {
    return this.repository.findByProductVariant(Id);
  }

  async findOne(id: string): Promise<ValueVariant> {
    const res: ValueVariant =
      await this.repository.findOne(id);
    if (!res) throw new NotFoundException(`No se encontró ${this.completeMessage}`);
    return res;
  }
  ...
  async remove(id: string): Promise<DeleteResult> {
    ...
    if (res.affected === 0)
      throw new NotFoundException(
        `No se encontro ${this.completeMessage}`,
      );
    return res;
    } catch (e) {
      throw new ConflictException(`No se puede eliminar ${this.completeMessage} porque esta relacionado con otra informacion y dejaria inconsistencias. Pruebe modificandolo. `)
    }
  }
}
