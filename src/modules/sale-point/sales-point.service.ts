...
export class SalePointService {
  ...
  async findCommerce(commerceId: string): Promise<SalePoint[]> {
    try {
      ...
      if (!res)
        throw new NotFoundException('No se encontraron los puntos de venta');
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
  ...
  async update(id: string, body: Partial<SalePoint>) {
    try {
      ...
      if (res.affected === 0)
        throw new NotFoundException('No se pudo encontrar el punto de venta');
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
  ...
  async remove(id: string) {
    try {
      ...
      throw new ConflictException(
        'Actualmente el punto de venta esta siendo usado. No se puede eliminar ya que dejaría información incompleta. Puede probar modificando el punto de venta si lo cree necesario. Error: ',
        error,
      );
    }
  }
