...
export class BoxCashService {
  ...
  async findAll(
    ...
    try {
      ...
      throw new ConflictException(
        'No puede asignar un mismo punto de venta a dos cajas distintas.',
      );
    }
  }
  ...
  async update(id: string, body: Partial<BoxCash>) {
    try {
      ...
      if (res.affected === 0)
        throw new NotFoundException('No se pudo encontrar la caja');
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
  ...
  async remove(id: string) {
    try {
      ...
      throw new ConflictException(
        'Actualmente la caja esta siendo usado. No se puede eliminar ya que dejaría información incompleta. Puede probar modificando la caja si lo cree necesario. Error: ',
        error,
      );
    }
  }
