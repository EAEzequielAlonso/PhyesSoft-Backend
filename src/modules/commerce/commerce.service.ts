...
export class CommerceService {
  ...
  async createCommerce(commerce: Partial<Commerce>): Promise<Commerce> {
    ...
    if (!commerceFind)
      throw new InternalServerErrorException(
        'Error al intentar crear el Comercio',
      );
    return commerceFind;
  }
  ...
  async updateCommerce(
    ...
    const commerceUpdate = await this.commerceRepository.updateCommerce(
      id,
      commerce,
    );
    if (commerceUpdate.affected === 0)
      throw new NotFoundException('Comercio a actualizar no encontrado');
    return id;
  }

  async deleteCommerce(id: string): Promise<string> {
    const commerceDelete = await this.commerceRepository.deleteCommerce(id);
    if (commerceDelete.affected === 0)
      throw new NotFoundException('Comercio a eliminar no encontrado');
    return id;
  }

  async unsubscribeCommerce(id: string): Promise<string> {
    const commerceUpdate =
      await this.commerceRepository.unsubscribeCommerce(id);
    if (commerceUpdate.affected === 0)
      throw new NotFoundException('Comercio a dar de baja no encontrado');
    return id;
  }
}
