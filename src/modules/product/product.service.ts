...
export class ProductService {
  ...
  constructor (private readonly productRepository: ProductRepository) {}
    ...
    async createProduct(product: Partial<Product>): Promise<Product> {
      ...
      if (!productFind) throw new InternalServerErrorException("Error al intentar crear el Producto")
      return productFind;
    }

    async updateProduct(id: string, product: Partial<Product>): Promise<string> {
      const productUpdate = await this.productRepository.updateProduct(id, product);
      if (productUpdate.affected === 0) throw new NotFoundException ("Producto a actualizar no encontrado")
      return id;
    }

    async deleteProduct(id: string): Promise<string> {
      const productDelete = await this.productRepository.deleteProduct(id);
      if (productDelete.affected === 0) throw new NotFoundException ("Producto a eliminar no encontrado")
      return id;
    }

    async unsubscribeProduct(id: string): Promise<string> {
      const productUpdate = await this.productRepository.unsubscribeProduct(id);
      if (productUpdate.affected === 0) throw new NotFoundException ("Producto a dar de baja no encontrado")
      return id;
    }
}
