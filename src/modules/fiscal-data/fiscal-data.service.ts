...
export class FiscalDataService {
  ...
  async findCommerce(commerceId:string): Promise<FiscalData[]> {
    try {
      ...
      if (!res) throw new NotFoundException("No se encontraron los datos Fiscales")
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error)
    }
  ...
  async create(body: Partial<FiscalData>): Promise<FiscalData> {
    try {
      ...
      if (!res) throw new InternalServerErrorException("No se pudo crear el dato fiscal")
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error)
    }
  ...
  async update(id: string, body: Partial<FiscalData>) {
    try {
      ...
      if (res.affected === 0) throw new NotFoundException("No se pudo encontrar el dato fiscal")
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error)
    }
  ...
  async remove(id: string) {
    try {
      ...
      throw new ConflictException(
        'Actualmente el dato fiscal esta siendo usada. No se puede eliminar ya que dejaría información incompleta. Puede probar modificando el dato fiscal si lo cree necesario. Error: ',error);
    }
  }
}
