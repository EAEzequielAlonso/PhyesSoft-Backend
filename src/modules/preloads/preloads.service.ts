...
export class PreloadsService {
  ...
  async preloadCategory (): Promise<void> {
    ...
    for (const category of preloadCategory) {
        const catFind = await this.categoryRepository.findOne({where: {category: category.category}})
        if (!catFind) {
  ...
  async preloadSubcategory (): Promise<void> {
    ...
    for (const subcategory of preloadSubcategory) {
        const subcatFind = await this.subcategoryRepository.findOne({where: {subcategory: subcategory.subcategory}})
        if (!subcatFind) {
            const catFind = await this.categoryRepository.findOne({where: {category: subcategory.category}})
            await this.subcategoryRepository.save({subcategory: subcategory.subcategory, categoryId:catFind.id});
            count++;
        }
    }
  ...
  async preloadBrand (): Promise<void> {
    ...
    for (const brand of preloadBrand) {
        const brandFind = await this.brandRepository.findOne({where: {brand: brand.brand}})
        if (!brandFind) {
  ...
  async preloadColor (): Promise<void> {
    ...
    for (const color of preloadColor) {
        const colorFind = await this.colorRepository.findOne({where: {color: color.color}})
        if (!colorFind) {
  ...
  async preloadSize (): Promise<void> {
    ...
    for (const size of preloadSize) {
        const sizeFind = await this.sizeRepository.findOne({where: {size: size.size}})
        if (!sizeFind) {
  ...
  async preloadModel (): Promise<void> {
    ...
    for (const model of preloadModel) {
        const modelFind = await this.modelRepository.findOne({where: {model: model.model}})
        if (!modelFind) {
            const brandFind = await this.brandRepository.findOne({where: {brand: model.brand}})
            await this.modelRepository.save({model: model.model, brandId: brandFind.id});
            count++;
        }
    }
  ...
  async preloadProduct (): Promise<void> {
    ...
    for (const product of preloadProduct) {
        const productFind = await this.productRepository.findOne({where: {description: product.description}})
        if (!productFind) {
            const subcatFind = await this.subcategoryRepository.findOne({where: {subcategory: product.subcategory}})
            const modelFind = await this.modelRepository.findOne({where: {model: product.model}})
            await this.subcategoryRepository.save({...product, subcategoryId:subcatFind.id, model: modelFind.id});
            count++;
        }
    }
