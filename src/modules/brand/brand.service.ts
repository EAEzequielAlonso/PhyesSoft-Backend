...
export class BrandService {
  ...
  async update(id: string, brand: Partial<Brand>) {
    try {
      ...
      if (res.affected === 0)
        throw new NotFoundException('No se pudo encontrar la marca');
      return res;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
  ...
  async remove(id: string) {
    try {
      ...
      throw new ConflictException(
        'Actualmente la marca esta siendo usada. No se puede eliminar ya que dejaría información incompleta. Puede probar modificando la marca si lo cree necesario. Error: ',
        error,
      );
    }
  }
