...
export class ModelService {
  ...
  async createModel (model: Partial<Model>): Promise<Model> {
    ...
    if (!modeloCreate) throw new InternalServerErrorException("No se pudo crear el modelo")
    return modeloCreate;   
  }
  ...
  async updateModel (id:string, model: Partial<Model>): Promise<Model> {
    ...
    if (modeloUpdate.affected === 0) throw new NotFoundException("No se encontro el modelo a actualizar")
  ...
  async deleteModel (id:string): Promise<Model> {
    ...
    if (!modelo) throw new NotFoundException("No se encontro el modelo a eliminar")
